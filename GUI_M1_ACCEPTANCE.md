...
9. 选择 `gray`，确认原图切换为灰度且不出现文件路径异常；此伪彩仅影响显示。
10. 在测试用 `configuration.json` 中配置逐轴 `objective_scan_bounds_mm`，输入一个超限计划，确认扫描前估算显示红色错误且开始按钮禁用；原输入值不得被程序自动修改。
11. 关闭程序，再用“打开已有扫描目录”或 `--open` 重开上一步目录；确认热图、剖面、原图和点信息可浏览。
12. 分别试用物镜 XY/XZ/YZ、单轴 range 和单轴 list；反向扫描需使用负步长，不可整除终点时程序不改变步长强行包含终点。
13. 扫描运行中关闭窗口，确认程序先停止工作线程再退出；已完成点仍可重新打开。
